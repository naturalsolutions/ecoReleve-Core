<?php
	...
	class TViewCameraTrapController extends AppController{
		...
		function detail(){
			...
			$recursive=0;
			$format="json";
			$this->loadModel('TViewCamera_trap');
			...
			$fields=array('Camera_trap_Obj_pk',		
				'id67@TCarac_cameratrap_Serial_Number',
			...
			if($editval!=""){
				$editval=str_replace("TViewCamera_trap_","",$editval);
				$fieldname="";
				$ltype="";
				$id_carac="";
				...
				if($fieldname!="Camera_trap_Obj_pk"){
				...
				if($labelcarac!="")
					$fields=array("Camera_trap_Obj_pk",'id67@TCarac_cameratrap_Serial_Number',"id41@TCaracThes_Model_Precision",
					"id42@TCaracThes_Company_Precision","id37@Comments");
				$iniresult=$this->TViewCamera_trap->find("first",array(
					'recursive'=>$recursive,
					'fields'=> $fields,
					'conditions'=> array('Camera_trap_Obj_pk'=>$id)
				));							
				$result=$iniresult;
				//print_r($result);
				//history case
				...
				if($labelcarac!=""){						
					...
					$countindval=count($iniresult['TViewCamera_trap']);
					$i=0;
					foreach($iniresult['TViewCamera_trap'] as $key=>$val){
						if($key=='Camera_trap_Obj_pk'){							
							$iniresult['TViewCamera_trap']['Id']=$iniresult['TViewCamera_trap']['Camera_trap_Obj_pk'];								
						}
						else{
							foreach($labelcarac as $name=>$label){
								if(strpos($key,$name)!==false){
									$iniresult['TViewCamera_trap'][$label]=$iniresult['TViewCamera_trap'][$key];	
								}
							}
						}
						unset($iniresult['TViewCamera_trap'][$key]);
						$i++;
						if($i>$countindval)
							break;
					}
					...
					*/
					...
					$result['TViewCamera_trap']=$result[0];
					unset($result[0]);
					foreach($result['TViewCamera_trap'] as $key=>$val){
						$editb=$this->editbouton("TViewCamera_trap","Camera_trap_Obj_pk",$fields,"TViewCamera_trap_".$key,$val);
						$result['TViewCamera_trap'][$key]=array($val,$editb);
					}
				}
