		...
		function detail(){
			...
			if($id!=""){				
				...
				//check if equipped or not
					...
					//edit boutton val
					foreach($result['TViewTrxSat'] as $key=>$val){
						$editb=$this->editbouton("TViewTrxSat","Trx_Sat_Obj_pk",$fields,"TViewTrxSat_".$key,$val);
						$result['TViewTrxSat'][$key]=array($val,$editb);
					}
				}	
				// $result[]= array($result);	
